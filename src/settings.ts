...
export class TodaySettingTab extends PluginSettingTab {
  ...
  private renderProjectsSection(containerEl: HTMLElement): void {
    ...
    const computeTarget = (
      clientY: number,
    ): { hoverIdx: number; after: boolean } | null => {
    ...
    const clearIndicators = (): void => {
      rows.forEach((r) =>
        r.removeClasses(["is-drop-above", "is-drop-below"]),
      );
    };
    ...
    list.addEventListener("dragover", (ev) => {
      ...
      // Don't show indicator at positions adjacent to the source (no-op).
      let dest = after ? hoverIdx + 1 : hoverIdx;
      if (dragSrcIdx < dest) dest -= 1;
